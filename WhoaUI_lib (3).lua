...
local AL,Flags,togByFlag,setByFlag,cbByFlag={},{},{},{},{}
...
local function _apply(data)
    ...
    task.defer(function()
        for k,v in pairs(data) do
            local fval = Flags[k]
            if setByFlag[k] then pcall(setByFlag[k], fval) end
            if togByFlag[k] and type(fval)=="boolean" then
                for _,fn in ipairs(togByFlag[k]) do pcall(fn,fval,true) end
            end
            if cbByFlag[k] then
                for _,fn in ipairs(cbByFlag[k]) do pcall(fn,fval) end
            end
        end
    end)
end
...
task.spawn(function()
    for _=1,3 do
        local ok,img=pcall(function()
            return Players:GetUserThumbnailAsync(LP.UserId,Enum.ThumbnailType.HeadShot,Enum.ThumbnailSize.Size60x60)
        end)
        if ok and img and img~="" then avImg.Image=img; break end
        task.wait(2)
    end
end)
...
return {
    ...
    Flags=Flags, setByFlag=setByFlag, togByFlag=togByFlag, cbByFlag=cbByFlag,
