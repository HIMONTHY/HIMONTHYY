...
local AL,Flags,togByFlag,cbByFlag,setByFlag={},{},{},{},{}
...
local function _apply(data)
    ...
    -- Fire registered callbacks so visuals update and scripts react
    task.defer(function()
        for k,v in pairs(data) do
            -- Sync slider visuals (redraws the bar and thumb)
            if setByFlag[k] then pcall(setByFlag[k], Flags[k]) end
            -- Sync toggle visuals
            if togByFlag[k] then
                for _,fn in ipairs(togByFlag[k]) do pcall(fn, Flags[k], true) end
            end
            -- Fire registered element callbacks
            if cbByFlag[k] then
                for _,fn in ipairs(cbByFlag[k]) do pcall(fn, Flags[k]) end
            end
        end
    end)
end
...
local function makeSection(parent,title)
    ...
    function api:AddCheckbox(cfg)
        ...
        if cfg.Callback and flg then if not cbByFlag[flg] then cbByFlag[flg]={} end; table.insert(cbByFlag[flg],cfg.Callback) end
...
return {
    ...
    Flags=Flags, cbByFlag=cbByFlag, togByFlag=togByFlag, setByFlag=setByFlag, pbNameLabel=pbNameLabel, avImg=avImg, wmFrame=wmFrame, wmNameLabel=wmNameLabel,
